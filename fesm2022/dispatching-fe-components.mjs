...
class CustomTimeInputFormComponent {
    ...
    get displayTime() {
        return (`${this.selectedHour || '--'}:${this.selectedMinute || '--'} ${this.selectedPeriod}` || null);
    }
    ngOnInit() {
        if (this.defaultTime) {
    ...
    toggleDropdown() {
        ...
        this.setFormValue();
        if (this.parentForm.get(this.controlName)?.invalid && !this.dropdownOpen()) {
            this.showErrors.set(true);
        }
    }
    setFormValue() {
        if (this.selectedHour && this.selectedMinute) {
            this.parentForm.get(this.controlName)?.setValue(this.displayTime);
        }
        else {
            this.parentForm.get(this.controlName)?.setValue(null);
        }
    ...
    confirmTime() {
        ...
        this.timeChange.emit(this.displayTime);
        this.setFormValue();
    }
    setTimeFromString(timeStr) {
        const match = timeStr.match(/^(\d{1,2}):(\d{2})\s*(AM|PM)$/i);
        if (match) {
            this.selectedHour = match[1].padStart(2, '0');
...
export { API_BASE_URL, AllowNumberOnlyDirective, ArabicOnlyDirective, AuthConstant, AuthInterceptor, AuthService, BlurBackdropDirective, ClickOutsideDirective, CommonHttpService, ComponentFormErrorConstant, CustomAppErrorComponent, CustomBreadcrumbComponent, CustomButtonComponent, CustomCalendarComponent, CustomCalenderFormComponent, CustomCheckBoxComponent, CustomCheckBoxFormComponent, CustomDropdownComponent, CustomDropdownFormComponent, CustomFieldsFormComponent, CustomFileUploadComponent, CustomFileViewerComponent, CustomInputComponent, CustomInputFormComponent, CustomLoadingSpinnerComponent, CustomModalComponent, CustomMultiSelectComponent, CustomMultiSelectFormComponent, CustomPaginationComponent, CustomPopUpComponent, CustomRadioGroupFormComponent, CustomSteppersContainerComponent, CustomSteppersControllersComponent, CustomSvgIconComponent, CustomTableComponent, CustomTextareaComponent, CustomTextareaFormComponent, CustomTimeInputFormComponent, CustomToastComponent, CustomToggleSwitchComponent, CustomToggleSwitchFormComponent, DispatchingFeComponentsService, EnglishOnlyDirective, ErrorInterceptor, I18nConstant, LoadingService, ModuleRoutes, NetworkConnectionInterceptor, OverlayPanelComponent, PermissionGuard, PlanStatusEnum, StepperService, ToastService, ToggleElementDirective, TranslationService, UserDataService, authGuard, b64toBlob, blobToB64, convertDateFormat, convertFileToBase64, convertFormGroupToFormData, diffTime, excelDateToJSDate, flattenTree, formatDate, formatDateWithTime, formatTimestamp, formatinitialTakeTime, generateRandomColor, generateUniqueNumber, getFormValidationErrors, isDocumentPath, isImagePath, isVedioPath, loadingInterceptor, logger, noAuthGuard, planPriorityEnum, planTypeNameEnum, someFieldsContainData, timeAgo };
