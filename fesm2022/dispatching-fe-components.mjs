...
class CustomTableComponent {
    ...
    constructor(sanitizer) {
        ...
        this.checkedActionEditSvg = this.sanitizer.bypassSecurityTrustHtml(actionEdiSquaretSvg);
        this.checkedActionDeleteSvg = this.sanitizer.bypassSecurityTrustHtml(actionDeleteSvg$1);
    }
    // ----------------------------
    // Expand logic
    // ----------------------------
    ...
    isCellEditing(row, col, rowIndex, isChild, parent) {
        ...
        return (this.editingCell.rowKey === rowKey &&
            ...
            (isChild ? this.editingCell.parentKey === this.getRowKey(parent, -1) : true));
    }
    ...
    hasSelectedChildren(parent) {
        if (!this.expandedChildAccessor)
            return false;
        return this.expandedChildAccessor(parent).some((c, i) => this.selection.children.has(this.getKey(c, i, parent)));
    }
    toggleParent(row, index, event) {
        event.stopPropagation();
        const key = this.getKey(row, index);
        if (this.hasSelectedChildren(row))
            return;
    ...
    toggleChild(child, index, parent, event) {
        event.stopPropagation();
        const parentKey = this.getKey(parent, -1);
        if (this.selection.parents.has(parentKey))
            return;
